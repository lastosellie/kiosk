<template>
    <v-card outlined>
        <v-card-title>
            RestaurantTable # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>

        <v-card-text>
            <div>
                <String label="TableId" v-model="item.tableId" :editMode="editMode" @change="change" />
            </div>
            <div>
            ...
            <div>
                <Date label="SaleDate" v-model="item.saleDate" :editMode="editMode" @change="change" />
            </div>
        </v-card-text>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/restaurantTables/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
