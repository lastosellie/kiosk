<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            restaurantTable # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            restaurantTable
        </v-card-title >        
        ...
        <v-card-text>
            <String label="TableId" v-model="value.tableId" :editMode="editMode" :inputUI="''"/>
...
<script>
    ...
    export default {
        name: 'RestauranttableRestaurantTable',
        components:{
        },
        props: {
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/restaurantTables'), this.value)
                        } else {
